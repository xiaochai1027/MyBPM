package com.i360r.bpm.business.handler.delivery.stationmanager.entry;
...
import com.i360r.bpm.service.rs.process.delivery.stationmanager.entry.api.InventoryVO;
...
public class StationManagerHandler extends AbstractBusinessHandler implements IStationManagerHandler {
	...
	public DeliveryStaffVO getDeliveryStaffByCode(String code) throws Exception {
		DeliveryStaffDetailResponse response = cdStaffInternalService.getDeliveryStaffById(code);
		checkResponseSuccess(response);
		return toVO(response.getDeliveryStaff());
	}
	...
	public void startEntryProcess(EmployeeCO employee, StationManagerHeadCountRequest request) throws Exception {
		...
		customVariables.put(StationManagerEntryConstants.KEY_STATIONMANAGER_CREATE_BY_CODE, employee.getCode());
		ProcessInstance processInstance = processHandler.startProcess(employee, request, 
				StationManagerEntryConstants.PROCESS_DEFINITION_KEY, customVariables, AccountType.EMPLOYEE);
		...
		if (!request.getJobTransfer()) {
			...
			runtimeService.setVariable(execution.getId(), 
					StationManagerEntryConstants.KEY_DELIVERY_STAFF_CODE, deliveryStaffCode);
