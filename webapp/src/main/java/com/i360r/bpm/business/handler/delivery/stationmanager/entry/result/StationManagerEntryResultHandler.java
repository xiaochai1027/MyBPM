package com.i360r.bpm.business.handler.delivery.stationmanager.entry.result;
...
public class StationManagerEntryResultHandler extends AbstractEngineHandler implements IProcessResultHandler {
	...
	public void onProcessPass(DelegateExecution execution) throws Exception {
		...
		StationManagerEntryDetailVO detail = ProcessUtility.getProcessObject(processVariables, StationManagerEntryDetailVO.class);
		if (!detail.getJobTransfer()) {
			...
			stationManagerHandler.createOfficialAccount(request);
		} else {
			...
			stationManagerHandler.transferPosition(request);
		}
	...
	public void onProcessNotPass(DelegateExecution execution) throws Exception {
		...
		Boolean jobTransfer = (Boolean) processVariables.get(StationManagerEntryConstants.KEY_JOB_TRANSFER);
		...
		if (!jobTransfer) {
			...
			String deliveryStaffCode = (String)processVariables.get(StationManagerEntryConstants.KEY_DELIVERY_STAFF_CODE);
			...
			stationManagerHandler.deliveryStaffDimission(request);
	...
	public String getProcessDefinitionKey() {
		return StationManagerEntryConstants.PROCESS_DEFINITION_KEY;
	}
